...
const formSchema = z.object({
	...
	selectedLists: z.array(z.string()).min(1, "Select at least one recipient list"),
	smtpConfigs: z
		.array(
			z.object({
	...
	return (
		<div className="container mx-auto py-8 px-4 max-w-4xl">
					...
					<FormField
						...
						render={({ field }) => (
							<FormItem>
								...
								<FormDescription>
									Choose which recipients to process first. Useful when adding new
									recipients during campaign.
								</FormDescription>
								<FormMessage />
							</FormItem>
						)}
			...
			<div className="mt-8 p-6 bg-muted/50 rounded-lg">
				...
				<p className="text-sm text-muted-foreground">
					Use <code className="bg-background px-2 py-1 rounded">{"{{name}}"}</code>{" "}
					and <code className="bg-background px-2 py-1 rounded">{"{{email}}"}</code>{" "}
