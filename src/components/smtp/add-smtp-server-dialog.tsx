  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const endpoint = isEditMode
        ? "/api/smtp-servers/update"
        : "/api/smtp-servers/create";
      const payload: any = {
        name,
        host,
      ...
      if (data.success) {
        ...
        toast.error(
          isEditMode
            ? "Failed to update SMTP server"
            : "Failed to add SMTP server",
          {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {isEditMode ? "Edit SMTP Server" : "Add SMTP Server"}
          </DialogTitle>
          <DialogDescription>
            {isEditMode
              ? "Update your email sending server configuration"
