...
> {
  return new Promise((resolve, reject) => {
    ...
    Papa.parse(fileContent, {
      ...
      skipEmptyLines: true,
      complete: (results) => {
        const recipients = (results.data as Record<string, unknown>[]).map(
          (row) => {
            const email = (row.email || row.Email || row.EMAIL) as string;
            const name = (row.name || row.Name || row.NAME) as
              | string
              | undefined;
    ...
    if (validRecipients.length > 0) {
      try {
        ...
        if (error && typeof error === "object" && "writeErrors" in error) {
          const writeErrors = (
            error as { writeErrors: Array<{ err: { errmsg: string } }> }
          ).writeErrors;
        ...
        const insertedCount =
          ...
          (error && typeof error === "object" && "writeErrors" in error
 ...
 */
export async function unsubscribeRecipient(recipientId: string, token: string) {
  // Verify token
