...
// MongoDB connection
const MONGODB_URI = process.env.DATABASE_URL || 'mongodb://localhost:27017/mail';
...
const BounceEvent = mongoose.models.BounceEvent || mongoose.model('BounceEvent', bounceEventSchema);
...
async function seedEmails(campaigns) {
  ...
  for (let i = 0; i < 50; i++) {
    ...
    if (email.bounced) {
      ...
      email.bounceReason = email.bounceType === 'hard' 
  ...
  for (const email of emails) {
    ...
    if (Math.random() < 0.7) {
      ...
      for (let i = 0; i < numOpens; i++) {
        const openTime = new Date(email.sentAt.getTime() + randomInt(60000, 86400000)); // 1 min to 24 hours after send
        const userAgent = i === 0 && Math.random() < 0.3 ? 'GoogleImageProxy' : randomElement(USER_AGENTS);
    ...
    if (Math.random() < 0.4) {
      ...
      for (let i = 0; i < numClicks; i++) {
        const clickTime = new Date(
          (email.firstOpenAt || email.sentAt).getTime() + randomInt(10000, 3600000)
        ...
        clickEvents.push({
          ...
          ipAddress: randomElement(IP_ADDRESSES),
          userAgent: randomElement(USER_AGENTS.filter(ua => !ua.includes('Proxy') && ua !== 'node')),
          isUnique: i === 0,
          timestamp: clickTime,
        });
